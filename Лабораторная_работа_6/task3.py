OUTPUT_FILE = "output.csv"  # название.сsv


def to_csv_file(filename: str, headers: list, rows: list, delimiter: str = ',', new_line: str = '\n'):
    """ Функция, которая записыватет передаваемые ей данные в формате csv.
    ...
    with open(filename, 'w') as f:  # открываем файл
        f.writelines(delimiter.join(headers))  # записываем заголовки с раздилителем
        f.write(new_line)  # разделяем строки
        f.writelines(delimiter.join(i) + new_line for i in rows)  # записываем данные
